...
import { runLuaTests } from "./luaTestRunner";
import lessons from "./data/luaLessons.json";

export default function LuaLesson() {
  const { level } = useParams();
  const navigate = useNavigate();
  const { currentLevel, setCurrentLevel } = useLevelProgress("luaCurrentLevel");
  ...
  const handleRun = async () => {
    ...
    setRunError(null);
    try {
      const { results: testResults, allPassed } = await runLuaTests(lesson, code);
  ...
  return (
    <MissionLesson
      ...
      mapRoute="/lua"
      completed={completed}
      onNext={() => navigate(nextLesson ? `/lua/${nextLesson.id}` : "/lua")}
    ...
    >
      <div className="space-y-4">
        <LuaEditor value={code} onChange={setCode} height={420} />
